...
class Snapshot_REST_Controller extends WP_REST_Controller {
	...
	public function get_snapshot_signed( WP_REST_Request $request ) {
		...
		if ( ! $this->signature_validator->is_valid( $key_id, $timestamp, $signature, $method, $path ) ) {
			return new WP_Error( 'pcc_bad_signature', __( 'Invalid or missing HMAC signature.', 'contextsnap-site-connector' ), array( 'status' => 401 ) );
		}
	...
	private function ensure_ip_allowed( WP_REST_Request $request ) {
		...
		return new WP_Error( 'pcc_forbidden_ip', __( 'Access denied for this IP address.', 'contextsnap-site-connector' ), array( 'status' => 403 ) );
	}
	...
	private function authorize_snapshot_request( WP_REST_Request $request ) {
		...
		if ( $auth && 0 === stripos( $auth, 'Bearer ' ) && empty( $options['allow_bearer'] ) ) {
			return new WP_Error( 'pcc_bearer_disabled', __( 'Bearer authentication is disabled for this endpoint.', 'contextsnap-site-connector' ), array( 'status' => 401 ) );
		}

		$user_id = get_current_user_id();
		if ( ! $user_id ) {
			return new WP_Error( 'pcc_unauthorized', __( 'Authentication required.', 'contextsnap-site-connector' ), array( 'status' => 401 ) );
		}
		...
		return new WP_Error( 'pcc_forbidden', __( 'You do not have permission to view the project snapshot.', 'contextsnap-site-connector' ), array( 'status' => 403 ) );
	}

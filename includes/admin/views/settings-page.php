...
<div class="wrap" role="region" aria-labelledby="pcc-heading">
	<h1 id="pcc-heading"><?php esc_html_e( 'Site Contextsnap', 'site-contextsnap' ); ?></h1>

	<p><?php esc_html_e( 'Configure CORS, rate limiting, caching, update metadata, and optional allow-lists. No secrets are stored in the database.', 'site-contextsnap' ); ?></p>

	<form action="options.php" method="post" novalidate>
		<?php
		settings_fields( 'pcc_settings' );
		do_settings_sections( 'pcc_settings' );
		submit_button( __( 'Save Changes', 'site-contextsnap' ) );
		?>
	</form>

	<hr />

	<h2><?php esc_html_e( 'HMAC Keys (Server-to-Server Access)', 'site-contextsnap' ); ?></h2>
	<p><?php esc_html_e( 'Define secrets only in wp-config.php. You can generate a key id and secret here for copy/paste. The secret is displayed once and not stored.', 'site-contextsnap' ); ?></p>
	...
	<div id="pcc-keygen" aria-live="polite" role="status">
		<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
			...
			<?php submit_button( __( 'Generate Key', 'site-contextsnap' ), 'secondary', 'submit', false ); ?>
		</form>
		...
		<?php if ( isset( $_GET['pcc_key'], $_GET['pcc_secret'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
			...
			<div class="notice notice-success" tabindex="0">
				<p><strong><?php esc_html_e( 'New key generated. Copy the values below now.', 'site-contextsnap' ); ?></strong></p>
				<label for="pcc-key-id"><?php esc_html_e( 'Key ID', 'site-contextsnap' ); ?></label>
				<input id="pcc-key-id" class="regular-text" type="text" readonly value="<?php echo esc_attr( $key_id ); ?>" />
				<button type="button" class="button" data-copy-target="#pcc-key-id"><?php esc_html_e( 'Copy', 'site-contextsnap' ); ?></button>

				<label for="pcc-secret" style="display:block;margin-top:8px;"><?php esc_html_e( 'Secret (displayed once)', 'site-contextsnap' ); ?></label>
				<input id="pcc-secret" class="regular-text" type="text" readonly value="<?php echo esc_attr( $secret ); ?>" />
				<button type="button" class="button" data-copy-target="#pcc-secret"><?php esc_html_e( 'Copy', 'site-contextsnap' ); ?></button>

				<p><?php esc_html_e( 'Add to wp-config.php (choose one):', 'site-contextsnap' ); ?></p>
		...
		(function(){
			document.addEventListener('click', function(e){
				...
				try {
					...
					btn.textContent = '<?php echo esc_js( __( 'Copied', 'site-contextsnap' ) ); ?>';
					setTimeout(function(){ btn.textContent = '<?php echo esc_js( __( 'Copy', 'site-contextsnap' ) ); ?>'; }, 1500);
				} catch (err) {}
			});
				...
				originsField.addEventListener('blur', function() {
					...
					if (origins.toLowerCase().indexOf('http://') !== -1) {
						if (!warningEl) {
							...
							warning.innerHTML = '<strong><?php echo esc_js( __( 'Warning', 'site-contextsnap' ) ); ?>:</strong> <?php echo esc_js( __( 'HTTP origins are insecure. Use HTTPS in production.', 'site-contextsnap' ) ); ?>';
	...
	<hr />
	<h2><?php esc_html_e( 'Examples', 'site-contextsnap' ); ?></h2>

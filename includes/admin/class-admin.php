<?php
/**
 * Admin settings for Contextsnap Site Connector.
 *
...
class Admin {
	...
	public function register_settings() {
		...
		add_settings_section(
			'pcc_general',
			__( 'General', 'contextsnap-site-connector' ),
			'__return_false',
			'pcc_settings'
		);
		add_settings_field( 'cors_enabled', __( 'Enable CORS', 'contextsnap-site-connector' ), array( $this, 'field_checkbox' ), 'pcc_settings', 'pcc_general', array( 'key' => 'cors_enabled' ) );
		add_settings_field( 'allowed_origins', __( 'Allowed Origins (CORS)', 'contextsnap-site-connector' ), array( $this, 'field_textarea' ), 'pcc_settings', 'pcc_general', array( 'key' => 'allowed_origins', 'placeholder' => "https://example.com\nhttps://ci.local" ) );
		add_settings_field( 'rate_limit_threshold', __( 'Rate limit: requests per window', 'contextsnap-site-connector' ), array( $this, 'field_number' ), 'pcc_settings', 'pcc_general', array( 'key' => 'rate_limit_threshold', 'min' => 1 ) );
		add_settings_field( 'rate_limit_window', __( 'Rate limit window (seconds)', 'contextsnap-site-connector' ), array( $this, 'field_number' ), 'pcc_settings', 'pcc_general', array( 'key' => 'rate_limit_window', 'min' => 1 ) );
		add_settings_field( 'cache_ttl', __( 'Cache max age (seconds)', 'contextsnap-site-connector' ), array( $this, 'field_number' ), 'pcc_settings', 'pcc_general', array( 'key' => 'cache_ttl', 'min' => 0 ) );
		add_settings_field( 'expose_updates', __( 'Expose update metadata', 'contextsnap-site-connector' ), array( $this, 'field_checkbox' ), 'pcc_settings', 'pcc_general', array( 'key' => 'expose_updates' ) );
		add_settings_field( 'expose_database_version', __( 'Expose database version', 'contextsnap-site-connector' ), array( $this, 'field_checkbox_with_description' ), 'pcc_settings', 'pcc_general', array( 'key' => 'expose_database_version', 'description' => __( 'Include database driver and version. Disable for minimal information disclosure.', 'contextsnap-site-connector' ) ) );

		add_settings_section(
			'pcc_access',
			__( 'Access Control', 'contextsnap-site-connector' ),
			function () {
				echo '<p>' . esc_html__( 'In addition to the capability pcc_read_snapshot (mapped to manage_options by default), optionally allow specific capabilities, user IDs, and IP addresses.', 'contextsnap-site-connector' ) . '</p>';
			},
			'pcc_settings'
		);
		add_settings_field( 'allow_caps', __( 'Allow capabilities (one per line)', 'contextsnap-site-connector' ), array( $this, 'field_textarea' ), 'pcc_settings', 'pcc_access', array( 'key' => 'allow_caps', 'placeholder' => "manage_options\nview_site_health" ) );
		add_settings_field( 'allow_user_ids', __( 'Allow user IDs (comma or newline separated)', 'contextsnap-site-connector' ), array( $this, 'field_textarea' ), 'pcc_settings', 'pcc_access', array( 'key' => 'allow_user_ids', 'placeholder' => "1\n42" ) );
		add_settings_field( 'allow_ips', __( 'Allow IPs (exact matches, one per line)', 'contextsnap-site-connector' ), array( $this, 'field_textarea' ), 'pcc_settings', 'pcc_access', array( 'key' => 'allow_ips', 'placeholder' => "127.0.0.1\n203.0.113.12" ) );
		add_settings_field( 'allow_bearer', __( 'Allow Bearer auth (if a JWT/OAuth plugin sets the current user)', 'contextsnap-site-connector' ), array( $this, 'field_checkbox' ), 'pcc_settings', 'pcc_access', array( 'key' => 'allow_bearer' ) );
	}
	...
	public function render() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to access this page.', 'contextsnap-site-connector' ) );
		}
	...
	public function handle_generate_key() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'Permission denied.', 'contextsnap-site-connector' ) );
		}
		...
		$redirect = add_query_arg(
			array(
				'page'      => 'contextsnap-site-connector',
